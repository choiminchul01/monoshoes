...
export async function POST(request: Request) {
    try {
        ...
        const { data, error } = await getResend().emails.send({
            from: 'monoshoes <onboarding@resend.dev>', // Resend 기본 발신자 주소 (도메인 인증 전까지 사용)
            to: [email],
            subject: `[monoshoes] 주문이 완료되었습니다. (주문번호: ${orderNumber})`,
            html: `
                <h1>주문이 완료되었습니다!</h1>
                <p>안녕하세요, monoshoes입니다.</p>
                ...
                <br/>
                <p>입금 계좌: 국민은행 123-456-789012 (예금주: 모노슈즈)</p>
